...
refresh.addEventListener('click', async () => {
  ...
  refreshStatus.textContent = 'Refresh started.';
  try {
    const response = await fetch('/api/refresh', { method: 'POST' });
    if (!response.ok && response.status !== 409) throw new Error('Refresh could not start');
    const started = Date.now();
    const poll = setInterval(async () => {
      const statusResponse = await fetch('/api/articles');
      const data = await statusResponse.json();
      render(data);
      if (data.run_status !== 'running' || Date.now() - started > 120000) {
        clearInterval(poll);
        refresh.disabled = false;
        refresh.classList.remove('busy');
        refresh.querySelector('span:last-child').textContent = 'Refresh now';
        refreshStatus.textContent = 'Refresh complete. Articles have been updated.';
      }
    }, 2000);
  } catch (error) {
    refresh.disabled = false;
